...
export default async function Sidebar() {
  ...
  const enrolledLessonIds = enrollments.flatMap((e) => e.course.lessons.map((l) => l.id));
  ...
  const enrolledCourses = enrollments.map((e) => {
    ...
    let sum = 0;
    for (const l of e.course.lessons) sum += percentByLesson.get(l.id) ?? 0;
    const percent = Math.max(0, Math.min(100, Math.round(sum / total)));
    const latest = latestByCourse.get(e.course.id) ?? null;
    return {
