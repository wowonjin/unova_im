...
import { basicAuthHeader, getTossBillingSecretKey } from "@/lib/toss";
...
export async function POST(req: Request) {
  ...
  if (!res.ok) {
    return NextResponse.json({ ok: false, error: "BILLING_ISSUE_FAILED", details: json }, { status: 400 });
  }

  return NextResponse.json({
    ok: true,
    billingKey: (json as any)?.billingKey ?? null,
